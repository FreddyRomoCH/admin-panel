...
export default function TableClients({ found, client }: TableClientsProps) {
    ...
    return (
        <tr key={client.project_name}>
            ...
            <td className="px-4 py-2">
                <Select name="select-status" id="select-status" options={PROJECT_STATUS} value={found.value} selectClass={`rounded-lg px-6 py-1 cursor-pointer hover:scale-105 transform transition-transform duration-150 ease-in-out`}  />
            </td>
